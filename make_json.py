...
META_IDEA_RE = re.compile(
    r'^\s*#?\s*(?:Big|Huge|Small)\s+Idea:\s*(.+?)(?:\s*\((.+?)\))?\s*$',
    re.IGNORECASE
)

META_CREDITS_RE = re.compile(
    r'^Credits:\s*(.+?)\s*$',
    re.IGNORECASE
)
...
def extract_metadata(md_text: str):
    ...
    for line in md_text.splitlines():
        ...
        # match: Big/Huge/Small Idea: NAME (STATUS)
        m = META_IDEA_RE.match(line)
        if m:
            name_candidate = m.group(1).strip()
            status_candidate = m.group(2).strip() if m.group(2) else None
        ...
        # match: Credits: AUTHOR
        m = META_CREDITS_RE.match(line)
        if m:
